...
{	
	...
	public static class DelaunayHelpers
	{
		public static List<LineSegment> VisibleLineSegments (List<Edge> edges)
		{
			...
			List<PtWSgmnts> points = new List<PtWSgmnts>();	

            for (int i = 0; i<edges.Count; i++) {
				Edge edge = edges [i];
				if (edge.visible) {
                    Vector2 p1 = edge.clippedEnds [Side.LEFT].Value;
                    Vector2 p2 = edge.clippedEnds [Side.RIGHT].Value;

                    PtWSgmnts newP1 = points.FirstOrDefault(p => p.pos.Equals(p1));
					if(newP1 is null)
						newP1 = new PtWSgmnts(p1);
...
//			}
        ...
        public static List<LineSegment> DelaunayLinesForEdges(List<Edge> edges)
        {
            ...
            {
                Edge edge = edges[i];
                var segment = edge.DelaunayLine();
                segments.Add(segment);
            }
            return segments;
        }

        public static List<LineSegment> DelaunayLinesForEdges (List<Edge> edges, List<PtWSgmnts> points)
		{
            List<LineSegment> segments = new List<LineSegment> ();
			Edge edge;
			for (int i = 0; i < edges.Count; i++) {
				edge = edges [i];
				edge.DelaunayLine(segments, points);
			}
			return segments;
		}
		...
		*/
		public static List<LineSegment> Kruskal (List<LineSegment> lineSegments, KruskalType type = KruskalType.MAXIMUM)
		{
			...
			for (int i = lineSegments.Count; --i > -1;) {
				...
				Node node0 = null;
				Node rootOfSet0;
				if (!nodes.ContainsKey (lineSegment.p0.pos)) {
					node0 = nodePool.Count > 0 ? nodePool.Pop () : new Node ();
					// intialize the node:
					rootOfSet0 = node0.parent = node0;
					node0.treeSize = 1;
					
					nodes [lineSegment.p0.pos] = node0;
				} else {
					node0 = nodes [lineSegment.p0.pos];
					rootOfSet0 = Find (node0);
				}
				
				Node node1 = null;
				Node rootOfSet1;
				if (!nodes.ContainsKey (lineSegment.p1.pos)) {
					node1 = nodePool.Count > 0 ? nodePool.Pop () : new Node ();
					// intialize the node:
					rootOfSet1 = node1.parent = node1;
					node1.treeSize = 1;
					
					nodes [lineSegment.p1.pos] = node1;
				} else {
					node1 = nodes [lineSegment.p1.pos];
