...
namespace Delaunay
{
	...
	public sealed class Edge
	{
		...
		public LineSegment DelaunayLine ()
		{
			...
			return new LineSegment (new PtWSgmnts(leftSite.Coord), new PtWSgmnts(rightSite.Coord));
		}

		public void DelaunayLine(List<LineSegment> existingSegments, List<PtWSgmnts> existingPoints)
		{
			PtWSgmnts lV = existingPoints.FirstOrDefault(v => v.pos == leftSite.Coord);
			if (lV is null)
			{ 
				lV = new PtWSgmnts(leftSite.Coord);
                existingPoints.Add(lV);
            }
            PtWSgmnts rV = existingPoints.FirstOrDefault(v => v.pos == rightSite.Coord);
            if (rV is null)
			{
                rV = new PtWSgmnts(leftSite.Coord);
				existingPoints.Add(rV);
            }

			var existingSegment = existingSegments.FirstOrDefault(s => s.ContainsAllEdgePointsPos(lV, rV));
			if (existingSegment is null)
				existingSegments.Add(new LineSegment(lV, rV));            
        }
        ...
        public LineSegment VoronoiEdge ()
		{
			...
			return new LineSegment (new PtWSgmnts(_clippedVertices[Side.LEFT].Value),
                                     new PtWSgmnts(_clippedVertices[Side.RIGHT].Value));
		}
...
//			}
		...
		public void ClipVertices (Rect bounds)
		{
			...
			if (vertex0 == _leftVertex) {
				_clippedVertices [Side.LEFT] = new Vector2 (MathF.Round(x0, digits), MathF.Round(y0, digits));
				_clippedVertices [Side.RIGHT] = new Vector2 (MathF.Round(x1, digits), MathF.Round(y1, digits));
			} else {
				_clippedVertices [Side.RIGHT] = new Vector2 (MathF.Round(x0, digits), MathF.Round(y0, digits));
				_clippedVertices [Side.LEFT] = new Vector2 (MathF.Round(x1, digits), MathF.Round(y1, digits));
			}
		}
